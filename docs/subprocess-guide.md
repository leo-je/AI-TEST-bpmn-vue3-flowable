# 实现BPMN多层流程设计器：路径导航栏 + 返回按钮功能
## 🎯 整体目标
实现一个支持 子流程嵌套导航 的 BPMN 流程设计器，包含以下两个核心 UI 功能：

顶部流程路径导航栏（Breadcrumb）：悬浮与页面左上角（left：56px，top：10px）
左上角“返回上一级”悬浮按钮：悬浮与页面左上角（left：10px，to：10px）
两者协同工作，提供直观的多层级流程设计体验。

## 📌 功能需求详述
### 一、流程路径导航栏（Breadcrumb）
#### 显示规则:
- 顶部流程路径导航栏（Breadcrumb）：悬浮与页面左上角（left：56px，top：10px）
- 默认显示当前主流程名称（从 bpmn:process 的 name 获取）
- 当用户通过 点击sub-process(collapsed)和Ad-hoc sub-process(collapsed)的"open 子流程id"按钮展开进入子流程 bpmn:SubProcess的设计界面时，自动追加子流程名称
- 路径格式：主流程 > 子流程1 > 子流程2

#### 交互行为
- 点击路径中的任意流程项 → 跳转回该层级
- 使用响应式栈 pathStack: Ref<ProcessPathItem[]> 管理路径
- 点击sub-process(collapsed)和Ad-hoc sub-process(collapsed) 的【open  子流程id】按钮进入子流程

### 二、“返回上一级”悬浮按钮
#### 显示规则
- 仅当 pathStack.length > 1 时显示
- 固定定位在页面左上角（left：10px，top：10px）
#### 交互行为
- 点击按钮 → 跳转回 pathStack[pathStack.length - 2]
#### 样式要求
```css
.bpmn-return-button {
  position: absolute;
  left: 10px;
  top: 10px;
  width: 40px;
  height: 40px;
  border-radius: 50%;
  background-color: rgba(0, 0, 0, 0.6);
  color: white;
  border: none;
  z-index: 999;
```

## 🧩 依赖上下文（已有基础）
项目使用 Vue 3（Composition API + <script setup>） + TypeScript
已集成 bpmn.js v18（BpmnJS 类型可用）
主流程 XML 已加载，bpmnModeler 实例已创建
子流程在 BPMN XML 中以标准 <bpmn:subProcess> 形式存在
判断子流程展开逻辑，参考
```ts
    const eventBus = modeler.get('eventBus');
    eventBus.on('root.set', function (e: any) {
      console.log('element', e.element);
    })

```
流程设计入口页面src/App.tsx
### 📦 技术要求
所有逻辑封装在 useProcessBreadcrumb 中
类型安全，避免 any（使用 bpmnModeler?.get(...) 安全调用）
可使用 UI 库（如 Element Plus），也可以使用原生 HTML + CSS
#### 路径栈数据结构
```ts
interface ProcessPathItem {
  id: string;
  name?: string;        // 优先显示 name，否则显示 id
  element: any;         // bpmn.js element 对象（含 businessObject）
}
```

### 📁 推荐文件结构
```sh
src/
├── components-/
│   └── bpmn-breadcrumb/
│       ├── useProcessBreadcrumb.ts  # 路径导航逻辑
│       ├── BpmnBreadcrumb.tsx       # 路径导航栏组件
│       └── BpmnReturnButton.tsx     # 返回按钮组件
```

### 测试方法
新增开始节点，再新增task元素，在task的contextpad中将task元素切换为sub-process(collapsed)，点击sub-process(collapsed)的“open 子元素id”按钮,进入子流程设计界面
必须满足要求：进入子流程，导航栏更新，并可点击，切换到对应设计视图，并显示返回按钮，点击可回到上层。请在每次修改后都要重新测试，确保必须实现要求的功能。use chrome-devtools-mcp,use context7

###🚫 禁止行为
❌ 不要自定义 contextPadProvider contextPad
❌ 不要修改 BPMN XML 结构（如添加 innerProcess）
❌ 不要重复实现子流程展开逻辑

### 🧪 交互流程示例
- 初始状态:路径：主流程名称,返回按钮：隐藏
- 点击子流程“open 子流程id”按钮,路径变为：主流程名称 > 子流程名称...,返回按钮：显示
- 点击“返回”按钮 或 点击“流程名称”，路径更新