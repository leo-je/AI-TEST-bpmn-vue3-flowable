...
export const resolveTypeName = (businessObject: ModdleElement): string => {
  ...
  // 检查eventDefinitions是否存在且不为空数组
  if (eventDefinitions && eventDefinitions.length > 0 && eventDefinitions[0]) {
    const eventDefinitionType = NameMapping[eventDefinitions[0].$type];
    return eventDefinitionType ? eventDefinitionType + nameMappingElement : nameMappingElement;
  }
  
  return nameMappingElement;
};
