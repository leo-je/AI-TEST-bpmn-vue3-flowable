      ...
      setValue(businessObject: ModdleElement, key: string, value: []): void {
        ...
        const properties = moddle.create(`flowable:Properties`, {
          values: value.map((attr: { name: string; value: unknown }) => {
            return moddle.create(`flowable:Property`, { name: attr.name, value: attr.value });
          }),
        });
        bpmnContext.updateExtensionElements('flowable:Properties', properties);
      },
    },
  },
};
    ...
    'extensionElements.formProperty': {
      ...
      getValue: (businessObject: ModdleElement): Array<FromPropertyElement> => {
        const formProperties = businessObject?.extensionElements?.values?.filter(
          (item: FromPropertyElement) => item.$type === 'flowable:FormProperty',
        );
      ...
      setValue(businessObject: ModdleElement, key: string, value: []): void {
        ...
        bpmnContext.updateExtensionElements('flowable:FormProperty', formProperties);
      },
    },
  },
};
